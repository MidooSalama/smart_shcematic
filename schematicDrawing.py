...
class MosfetN(elm.Element):
    # This class build a custom element (Mosfet of N type)for the SchemDraw
...
class MosfetP(elm.Element):
    # This class build a custom element (Mosfet of P type)for the SchemDraw
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        factor = 0.5
        self.segments.append(Segment([[0, 1], [0, -1]]))
...
class Schematic():
    # This class Draw the Schematic with its basic building blocks
    def __init__(self):
        self.redraw()
